...
class DefaultManifestProvider {
    async updateCapability(ctx, inputs, capability) {
        const res = await utils_1.manifestUtils.updateCapability(inputs.projectPath, capability);
        if (res.isErr())
            return teamsfx_api_1.err(res.error);
        return teamsfx_api_1.ok(teamsfx_api_1.Void);
    }
    async deleteCapability(ctx, inputs, capability) {
        const res = await utils_1.manifestUtils.deleteCapability(inputs.projectPath, capability);
        if (res.isErr())
            return teamsfx_api_1.err(res.error);
        return teamsfx_api_1.ok(teamsfx_api_1.Void);
    }
    async capabilityExceedLimit(ctx, inputs, capability) {
        return await utils_1.manifestUtils.capabilityExceedLimit(inputs.projectPath, capability);
    }
    async addCapabilities(ctx, inputs, capabilities) {
        const res = await utils_1.manifestUtils.addCapabilities(inputs, capabilities);
        if (res.isErr())
            return teamsfx_api_1.err(res.error);
        return teamsfx_api_1.ok(teamsfx_api_1.Void);
    }
