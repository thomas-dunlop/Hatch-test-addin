...
async function scaffoldSPFx(context, inputs, outputFolderPath) {
    ...
    try {
        ...
        yoEnv.PATH = tools_1.isYoCheckerEnabled()
            ? `${await (await yoChecker.getBinFolders()).join(path.delimiter)}${path.delimiter}${(_d = process.env.PATH) !== null && _d !== void 0 ? _d : ""}`
            : process.env.PATH;
        const args = [
        ...
        if (error.name === "DependencyInstallFailed") {
            ...
            (_e = context.logProvider) === null || _e === void 0 ? void 0 : _e.error(`We've encountered some issues when trying to install prerequisites under HOME/.fx folder.  Learn how to remediate by going to this link(aka.ms/teamsfx-spfx-help) and following the steps applicable to your system: \n ${yoMessage} \n ${generatorMessage}`);
            (_f = context.logProvider) === null || _f === void 0 ? void 0 : _f.error(`Teams Toolkit recommends using ${yoInfo.displayName} ${genInfo.displayName}`);
        }
        if (error.message &&
            error.message.includes("'yo' is not recognized as an internal or external command")) {
            (_g = context.logProvider) === null || _g === void 0 ? void 0 : _g.error("NPM v6.x with Node.js v12.13.0+ (Erbium) or Node.js v14.15.0+ (Fermium) is recommended for spfx scaffolding and later development. You can use correct version and try again.");
        }
