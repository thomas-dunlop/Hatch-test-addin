...
class AzureHosting {
    ...
    async generateBicep(bicepContext) {
        ...
        const modules = await Promise.all(bicepFiles.map(async (filename) => await __1.generateBicepFromFile(path_1.default.join(bicepTemplateDir, filename), context)));
    ...
    async updateBicep(bicepContext) {
        ...
        const context = utils_1.getHandlebarContext(bicepContext, this.hostType);
        const bicepTemplateDir = this.getBicepTemplateFolder();
        const module = await __1.generateBicepFromFile(path_1.default.join(bicepTemplateDir, bicepFile), context);
